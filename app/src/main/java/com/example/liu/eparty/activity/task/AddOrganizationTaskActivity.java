...
public class AddOrganizationTaskActivity extends BaseActivity {

    @BindView(R.id.add_task_organization_title)
    EditText title;
    @BindView(R.id.add_task_organization_start_time)
    TextView startTime;
    @BindView(R.id.add_task_organization_end_time)
    TextView endTime;
    @BindView(R.id.add_task_organization_organization)
    TextView organization;
    @BindView(R.id.add_task_organization_attachment_name)
    TextView attachmentName;
    @BindView(R.id.add_task_organization_content)
    EditText content;

    private MaterialDialog materialDialog;
    private List<String> path;
    private String filePath;
    private ArrayList<Integer> organizationIds = new ArrayList<>();
    ...
    @OnClick(R.id.add_task_organization_start_time)
    public void chooseStartTime() {
        DateTimePicker.choose(this, startTime);
    }

    @OnClick(R.id.add_task_organization_end_time)
    public void chooseEndTime() {
        DateTimePicker.choose(this, endTime);
    }

    @OnClick(R.id.add_task_organization_organization)
    public void chooseOrganization() {
        Intent intent = new Intent(this, MultiChooseOrganizationActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 0 && resultCode == RESULT_OK) {
            organizationIds = data.getIntegerArrayListExtra("organizationIds");
            ArrayList<String> organizationNames = data.getStringArrayListExtra("organizationNames");
            if (organizationNames.size() == 0) {
                organization.setText("请选择");
            } else {
                organization.setText(ConvertUtil.listToString(organizationNames));
            }
        }
    }

    @OnClick(R.id.add_task_organization_attachment)
    public void chooseAttachment(){
        ...
        new Thread() {
            @Override
            public void run() {
                path = FileUtil.getSpecificTypeOfFile(AddOrganizationTaskActivity.this);
    ...
    @OnClick(R.id.add_task_organization_issue)
    public void issue(){
        if (check()){
            MyOkHttpUtil.post("SentTast")
                    ...
                    .addParams("uid", ConvertUtil.listToString(organizationIds))
    ...
    private boolean check() {
        ...
        if (organization.getText().toString().equals(getString(R.string.choose))) {
            ToastUtil.show(this, "最少选择一个组织");
            return false;
        }
